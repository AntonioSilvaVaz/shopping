...
async function ValidateRegisterAndLogin(ctx: Context, next: Next) {
  ...
  const isValidPassword = !validator.isEmpty(
...
async function ValidateUpdate(ctx: Context, next: Next) {
  ...
  const isValidName = !validator.isEmpty(
...
async function ValidateCreateItem(ctx: any, next: Next) {
  ...
  const isValidDescription = !validator.isEmpty(
    info.product_description ? info.product_description : ""
  );
  const isValidPrice = !validator.isEmpty(
    info.product_price ? info.product_price : ""
  );
  const isValidRegion = !validator.isEmpty(
    info.product_region ? info.product_region : ""
  );
...
async function ValidateUpdateItem(ctx: Context, next: Next) {
  ...
  const isValidName = !validator.isEmpty(
    info.product_name ? info.product_name : "Hello"
  );
  const isValidDescription = !validator.isEmpty(
    info.product_description ? info.product_description : "Hello"
  );
  const isValidPrice = !validator.isEmpty(
    info.product_price ? info.product_price : "Hello"
  );
  const isValidRegion = !validator.isEmpty(
    info.product_region ? info.product_region : "Hello"
  );
  const isValidPictures = !validator.isEmpty(
    info.product_pictures ? info.product_pictures : "Hello"
  );
...
async function ValidateItem(ctx: Context, next: Next) {
  let { item_id } = ctx.request.body as { item_id: string };
